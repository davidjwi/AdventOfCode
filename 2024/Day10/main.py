...
def DFS(CurrPoint: Point, cache: dict) -> int:
    # its in the cache so return the number of trails from that point
    if CurrPoint in cache:
        print(CurrPoint)
        print(cache[CurrPoint])
        return cache[CurrPoint]
    
    if CurrPoint.val == 9:
        print('Curr Point is 9 return 1')
        return 1

    trail_count = 0
    for neighboring_point in CurrPoint.neighbors.values():
        if neighboring_point.val == CurrPoint.val + 1:
            print(f'Recurse to {neighboring_point}')
            trail_count += DFS(neighboring_point, cache)

    print(f'Adding CurrPoint({CurrPoint.x},{CurrPoint.y}) with count of {trail_count} to cache')
    cache[CurrPoint] = trail_count
    return trail_count
...
def main() -> int:
    ...
    for x in range(0, len(topo_map)):
        for y in range(0, len(topo_map[0])):
            ...
            currPointAnswer = 0
            if CurrPoint.val == 0:
                print(f'CurrPoint{CurrPoint}')
                currPointAnswer += DFS(CurrPoint, trail_cache)
                print(f'Trailhead{CurrPoint} has {currPointAnswer} trail(s)')
                answer += currPointAnswer
