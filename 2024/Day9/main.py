def create_disk(filepath: str) -> list:
...
def get_next_freespace(disk: list, block_size: int, search_until: int) -> int | None:
    i = 0
    while i < search_until:
        if disk[i] == '.':
            j = start_index
            while j + 1 < len(disk) and disk[j + 1] == '.':
                j += 1
            freespace_size = (j - start_index) + 1
            if freespace_size >= block_size:
                return start_index
            i = j + 1
        i += 1
    return None

def get_next_rightside_fileblock(rightPointer: int, disk_list: list, checkedNums: set) -> tuple | None:
    '''Starting at the pointer and moving right find the next file block and return the size'''
    for i in range(rightPointer, -1, -1):
        char = disk_list[i]
        if char != '.' and char not in checkedNums:
            file_id = char
            block_start_index = i
            while block_start_index > 0 and disk_list[block_start_index - 1] == file_id:
                block_start_index -= 1
            block_size = (i - block_start_index) + 1
            return (file_id, block_start_index, block_size)
    return None
...
def main():
    disk = create_disk(filepath='Day9/sample_input.txt')
    ...
    while right_pointer > -1:
        if disk[right_pointer] != '.' and disk[right_pointer] not in checkedNums:
            left_pointer = 0
            data = get_next_rightside_fileblock(right_pointer, disk, checkedNums)
            if data is None:
                break
